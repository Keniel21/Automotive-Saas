...
// --------------------------------------------------------------------------
// B. BANCO DE DADOS LOCAL (FALLBACK / MODO DEMONSTRAÇÃO)
// --------------------------------------------------------------------------
let estoque = [
...
let leads = [
    {
        id: 1, name: "Carlos Alberto Santos", phone: "(11) 98765-4321", interestCarId: 1, origin: "Instagram", status: "negociação", lastContactDays: 2, nextAction: "Enviar simulação de financiamento hoje à tarde",
        interactions: [
            { date: "20/05/2026", text: "Cliente entrou em contato via direct perguntando sobre Jeep Compass. Gostou da KM." },
            { date: "21/05/2026", text: "Fizemos contato via WhatsApp. Enviou proposta de veículo na troca. Em análise." }
        ]
    },
    {
        id: 2, name: "Mariana Costa", phone: "(21) 99123-4567", interestCarId: 2, origin: "WhatsApp", status: "novo lead", lastContactDays: 0, nextAction: "Fazer primeiro contato de apresentação",
        interactions: [
            { date: "22/05/2026", text: "Lead gerado automaticamente via botão flutuante do site. Carro de interesse: Corolla." }
        ]
    },
    {
        id: 3, name: "Ricardo Mendes", phone: "(11) 97321-8901", interestCarId: 3, origin: "OLX/Webmotors", status: "sem resposta", lastContactDays: 14, nextAction: "Fazer última tentativa de contato antes de arquivar",
        interactions: [
            { date: "08/05/2026", text: "Cliente demonstrou interesse no Civic através da OLX." },
            { date: "10/05/2026", text: "Enviei fotos adicionais do Civic. Cliente visualizou e não respondeu." },
            { date: "15/05/2026", text: "Fiz cobrança de retorno. Sem resposta até o momento." }
        ]
    },
    {
        id: 4, name: "Ana Beatriz Ramos", phone: "(31) 98456-1122", interestCarId: 4, origin: "Indicação", status: "contato realizado", lastContactDays: 4, nextAction: "Agendar vistoria cautelar na oficina do mecânico dela",
        interactions: [
            { date: "18/05/2026", text: "Indicada pelo primo dela (Pedro). Procura carro econômico, gostou do Polo." },
            { date: "20/05/2026", text: "Conversamos sobre preço. Pediu para levar o carro para avaliação amanhã." }
        ]
    },
    {
        id: 5, name: "Felipe Andrade Lins", phone: "(11) 99345-6789", interestCarId: 5, origin: "OLX/Webmotors", status: "sem resposta", lastContactDays: 32, nextAction: "Mudar status para perdido se não responder",
        interactions: [
            { date: "20/04/2026", text: "Perguntou sobre condições de repasse no Onix." },
            { date: "22/04/2026", text: "Enviado valor à vista de repasse. Cliente sumiu." }
        ]
    }
];
...
// --------------------------------------------------------------------------
// C. CONTROLE DE FLUXO DE LOGIN E INICIALIZAÇÃO
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// E. RENDERIZADORES E NAVEGAÇÃO DE TELA (SPA)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// G. GERENCIAMENTO DE GRÁFICOS (CHART.JS)
// --------------------------------------------------------------------------
function initCharts() {
    ...
    if (ctxSales) {
        ...
        chartSalesProfitInstance = new Chart(ctxSales, {
            type: 'bar',
            data: {
                labels: ['Fev 26', 'Mar 26', 'Abr 26', 'Maio 26'],
                datasets: [
                    {
                        label: 'Faturamento Bruto (x1000 R$)',
                        data: [120, 195, 140, 182],
                    ...
                    {
                        label: 'Lucro Líquido Real (x1000 R$)',
                        data: [14, 23, 16, 21],
    ...
    if (ctxLeads) {
        ...
        chartLeadsOriginInstance = new Chart(ctxLeads, {
            type: 'doughnut',
            data: {
                labels: Object.keys(originCounts).length > 0 ? Object.keys(originCounts) : ["Nenhum"],
                datasets: [{
                    data: Object.values(originCounts).length > 0 ? Object.values(originCounts) : [1],
                    backgroundColor: ['#2563EB', '#8B5CF6', '#F59E0B', '#10B981', '#6B7280'],
                    borderWidth: 2,
                    borderColor: '#1F2937'
                }]
            },
    ...
    if (ctxGiro) {
        ...
        chartGiroEstoqueInstance = new Chart(ctxGiro, {
            type: 'bar',
            data: {
                labels: ['Venda Convencional', 'Repasse Rápido'],
                datasets: [{
                    label: 'Dias Médios em Estoque',
                    data: [avgConv.toFixed(0), avgRep.toFixed(0)],
...
function renderEstoqueTable(filterStatus = 'todos', searchQuery = '') {
    ...
    filteredCars.forEach(car => {
        ...
        tr.innerHTML = `
            <td>
                <div class="car-cell">
                    <div class="car-thumb"><i data-lucide="car"></i></div>
                    <div class="car-info">
                        <h4>${car.model}</h4>
                        <span>ID: #${car.id}</span>
                    </div>
            ...
            <td>
                <div style="font-weight:600; color: var(--green-profit)">${formatCurrency(lucroEst)}</div>
                <div style="font-size:11px; color: var(--text-muted)">${margemEst.toFixed(1)}% margem</div>
            </td>
            ...
            <td style="text-align: center;">
                ${car.status !== 'vendido' 
                    ? `<button class="btn btn-success" style="padding: 5px 10px; font-size:11px" onclick="openSellCarModal(${car.id})"><i data-lucide="badge-dollar-sign" style="width:12px;height:12px;"></i> Vender</button>`
                    : `<span style="color:var(--text-muted);font-size:11px"><i data-lucide="check"></i> Vendido</span>`
                }
            </td>
        `;
    ...
    filteredLeads.forEach(lead => {
        ...
        card.innerHTML = `
            ...
            <div class="lead-footer">
                ...
                <button class="lead-actions-btn" onclick="openTimelineModal(${lead.id})"><i data-lucide="message-square-plus"></i></button>
            </div>
        `;
...
async function dropLead(event, newStatus) {
    ...
    if (lead && lead.status !== newStatus) {
        ...
        if (newStatus === 'fechado') {
            openSellCarModal(lead.interestCarId, lead.name);
        } else {
            updateApplicationState();
        }
    }
...
function renderLeadsList(searchQuery = '') {
    ...
    filteredLeads.forEach(lead => {
        ...
        tr.innerHTML = `
            ...
            <td style="text-align: center;">
                <button class="btn btn-secondary" style="padding: 4px 8px; font-size:11px" onclick="openTimelineModal(${lead.id})">
                    <i data-lucide="clock" style="width:12px;height:12px;"></i> Ações
                </button>
            </td>
        `;
...
function renderVendasTable() {
    ...
    [...vendas].reverse().forEach(v => {
        ...
        tr.innerHTML = `
            ...
            <td style="text-align: center; color: var(--blue-primary)">
                <i data-lucide="file-check" style="cursor:pointer" title="Ver Recibo da Venda"></i>
            </td>
        `;
        body.appendChild(tr);
    });
...
function renderFinanceiro() {
    ...
    const custoVendas = vendas.reduce((sum, v) => {
...
function initFinanceChart() {
    ...
    const categoriesSum = {};
    despesas.forEach(d => {
        categoriesSum[d.category] = (categoriesSum[d.category] || 0) + d.val;
    });
    ...
    chartFinanceInstance = new Chart(ctx, {
        type: 'pie',
        data: {
            labels: Object.keys(categoriesSum).length > 0 ? Object.keys(categoriesSum) : ["Nenhum Gasto"],
            datasets: [{
                data: Object.values(categoriesSum).length > 0 ? Object.values(categoriesSum) : [1],
                backgroundColor: ['#EF4444', '#F59E0B', '#2563EB', '#8B5CF6'],
                borderWidth: 1,
                borderColor: '#1F2937'
            }]
        },
...
// --------------------------------------------------------------------------
// M. PREENCHIMENTO DE FORMULÁRIOS & SELECTS
// --------------------------------------------------------------------------
function populateCarSelects() {
    const selectInterest = document.getElementById("lead-interest");
...
function populateDespesaCarSelect() {
...
function populateAgendaCarSelect() {
    const select = document.getElementById("age-car");
    if (!select) return;
...
// --------------------------------------------------------------------------
// N. MODAIS E PROCESSOS DE INSERÇÃO / SALVAMENTO
// --------------------------------------------------------------------------

// 1. Modais de Leads
function openAddLeadModal() {
    populateCarSelects();
    document.getElementById("modal-add-lead").classList.add("active");
}
function closeAddLeadModal() {
    document.getElementById("modal-add-lead").classList.remove("active");
...
// 2. Modais de Veículos
function openAddCarModal() {
    document.getElementById("modal-add-car").classList.add("active");
}
...
function openTimelineModal(leadId) {
    ...
    lead.interactions.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <p class="timeline-item-desc">${item.text}</p>
        `;
...
function closeTimelineModal() {
    document.getElementById("modal-lead-timeline").classList.remove("active");
...
// 4. Modais de Fechamento de Venda
...
function closeSellModal() {
    document.getElementById("modal-sell-car").classList.remove("active");
    document.getElementById("form-sell-car").reset();
}
...
async function saveSale(event) {
    ...
    if (car) {
        ...
        if (isCloudActive) {
            ...
            const newSale = { id: vendas.length + 1, carId, client, sellPrice, date, profit, margin, type };
            vendas.push(newSale);
...
// 5. Modais de Despesas
function openAddDespesaModal() {
    populateDespesaCarSelect();
    document.getElementById("modal-add-despesa").classList.add("active");
}
function closeAddDespesaModal() {
    document.getElementById("modal-add-despesa").classList.remove("active");
...
function openAddAgendaModal() {
    populateAgendaCarSelect();
    document.getElementById("modal-add-agenda").classList.add("active");
}
function closeAddAgendaModal() {
    document.getElementById("modal-add-agenda").classList.remove("active");
